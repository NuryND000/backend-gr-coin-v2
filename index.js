...
//REGISTER CSV
app.post('/register/csv', upload.single('file'), async (req, res) => {
    ...
    .on('end', async () => {
      try {
        ...
        for (const user of users) {
          const { username, alamat, wilayah, tlp, ewallet, password, role } = user;
          const rowNum = user._rowNumber;

          // Cek data kosong
          if (!username || !alamat || !wilayah || !tlp || !ewallet || !password) {
            skippedRows.push({ row: rowNum, reason: 'Data tidak lengkap' });
            continue;
          }
          ...
          if (existing) {
            // Periksa mana yang duplikat
if (existing.tlp === tlp) {
              skippedRows.push({ row: rowNum, reason: 'Duplikat tlp' });
            } else if (existing.ewallet === ewallet) {
              skippedRows.push({ row: rowNum, reason: 'Duplikat ewallet' });
            } else {
              skippedRows.push({ row: rowNum, reason: 'Duplikat data' });
            }
            continue;
          }
        ...
        fs.unlinkSync(filePath);
        res.status(500).json({ error: 'Error processing CSV data', detail: err.message });
      }
    });
});
...
app.get("/coinexchange", authMiddleware, async (req, res) => {
  ...
  app.get("/coinexchange/all", authMiddleware, async (req, res) => {
    try {
      ...
      const exchanges = await prisma.coinExchange.findMany({
      include: {
        user: true,  // ini akan fetch data user yang relasi dengan userId
      },
    });
      res.json(exchanges);
    } catch (error) {
